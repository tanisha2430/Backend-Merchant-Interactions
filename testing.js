...
app.post('/tasks', async (req, res) => {
    const { merchantId, businessType, assignedTo, followUpDate, status, taskTitle, taskDescription, reminderDate, reminderTime } = req.body;

    // Check if all required fields are provided
    if (!merchantId || !businessType || !assignedTo || !followUpDate || !status || !taskTitle) {
        return res.status(400).json({ message: 'All required fields must be provided' });
    }
...
app.get('/tasks', async (req, res) => {
    const { merchantId, status, assignedTo, businessType } = req.query;
    ...
    try {
        ...
        if (merchantId) {
            filter.merchantId = mongoose.Types.ObjectId(merchantId); // Ensure merchantId is a valid ObjectId
        }
        if (status) {
            filter.status = status;
        }
